...
exports.handler = async (event, context) => {
  ...
  if(event.path === "/dishes" && event.httpMethod === "POST") {
    const body = JSON.parse(event.body);
    const { id, name, description, image, price, restaurantId } = body;
    if (!id || !name || !description || !image || !price || !restaurantId) {
      const response = composeResponse(400, {
        message: "Missing required fields",
      });
      return response;
    }
    const dish = await createDish(id, name, description, image, price, restaurantId);
    console.log(dish)
  ...
  if(event.path === "/restaurants" && event.httpMethod === "POST") {
    const body = JSON.parse(event.body);
    const { id, name, description, image, lat, lng, address, categoryId, deliveryFee } = body;
    ...
    const restaurant = await createRestaurant(id, name, description, image, lat, lng, address, categoryId, deliveryFee);
