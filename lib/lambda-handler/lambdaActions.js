...
async function createDish(id, name, description, image, price) {
  const res = await dynamodb
    .putItem({
      TableName: process.env.DISHES_TABLE_NAME,
      Item: {
...
async function createRestaurant(id, name, description, image, lat, lng, address, rating, reviews, categoryId, dishes, deliveryFee) {
  const res = await dynamodb
    .putItem({
